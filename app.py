...
_AI_NOTICE_HTML = (
    '<div class="digest-ai-notice">Note: some content on this page — including '
    'eNotice summaries, images, and tags — is generated by AI and may be '
    'inaccurate or incomplete. Please refer to the original eNotice for '
    'authoritative information.</div>')


def _render_ai_notice():
    """Transparency/liability caveat shown near the bottom of the Archive and
    each digest, noting that some page content is AI-generated."""
    st.markdown(_AI_NOTICE_HTML, unsafe_allow_html=True)
...
def render_digest_freq_view(freq, digest, selected_norm, unit_info):
    ...
    def _freq_footer():
        """Archive + preferences links, the AI-content notice, and copyright,
        shown on every path."""
        st.button("View your eNotice Archive", key="view_archive_freq",
                  on_click=_go_digest)
    ...
    with c_logo:
        if _LOGO_PATH.exists():
            st.image(str(_LOGO_PATH), width=150)
    with c_title:
        st.markdown('<div class="digest-title">Your IEEE eNotice '
                    f'{freq.replace(" Digest", "")} Digest</div>',
                    unsafe_allow_html=True)
...
st.markdown(
    ...
    .digest-copyright { text-align: center; color: #777; font-size: 1rem;
                        margin-top: 0.3rem; }
    /* Footer text links ("Manage your preferences", "View your eNotice
       Archive") rendered as centered inline text links. */
    .st-key-manage_prefs button, .st-key-manage_prefs_freq button,
    .st-key-view_archive_freq button {
    ...
    .st-key-manage_prefs_freq button:hover,
    .st-key-view_archive_freq button:hover { text-decoration: underline; }
    .st-key-manage_prefs, .st-key-manage_prefs_freq,
    .st-key-view_archive_freq {
        width: 100% !important; display: flex; justify-content: center; }
    .digest-ai-notice { text-align: center; color: #888; font-size: 0.8rem;
                        font-style: italic; line-height: 1.4; max-width: 640px;
